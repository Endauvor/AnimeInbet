...
def visvid(dict, inter_frames=20):

    img1 = ((dict['image0'][0].cpu().permute(1, 2, 0).float().numpy() + 1.0) * 255 / 2).astype(np.uint8).copy()
    #img2 = ((dict['image1'][0].cpu().permute(1, 2, 0).float().numpy() + 1.0) * 255 / 2).astype(np.uint8).copy()
